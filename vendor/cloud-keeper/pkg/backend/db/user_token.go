...
type UserToken struct {
	...
	Name       string    `column:"name" gorm:"column:name"`
}

func CheckToken(handle storage.Interface, token string) (int64, error) {

	fileds := []string{"id", "token", "user_id", "create_time", "expire_time", "name"}
	query := string("token = ?")
...
func CreateToken(handle storage.Interface, token string, uid int64, name string) error {
