...
import (
	...
	"gofreezer/pkg/auth/authenticator"

	"gofreezer/pkg/auth/authenticator/bearertoken"
	"gofreezer/pkg/auth/group"

	//"apistack/pkg/serviceaccount"

	certutil "apistack/pkg/util/cert"
	"apistack/plugin/pkg/auth/authenticator/password/keystone"
	"apistack/plugin/pkg/auth/authenticator/password/passwordfile"
	"apistack/plugin/pkg/auth/authenticator/request/anonymous"
	"apistack/plugin/pkg/auth/authenticator/request/basicauth"
	"apistack/plugin/pkg/auth/authenticator/request/headerrequest"
	"apistack/plugin/pkg/auth/authenticator/request/union"
	"apistack/plugin/pkg/auth/authenticator/request/x509"
	"apistack/plugin/pkg/auth/authenticator/token/anytoken"
	"apistack/plugin/pkg/auth/authenticator/token/innerhook"
	"apistack/plugin/pkg/auth/authenticator/token/oidc"
	"apistack/plugin/pkg/auth/authenticator/token/tokenfile"
...
type AuthenticatorConfig struct {
	...
	//append innerhook
	InnerHookFunc innerhook.InnerHookFunc
}
...
func New(config AuthenticatorConfig) (authenticator.Request, *spec.SecurityDefinitions, error) {
	...
	if config.InnerHookFunc != nil {
		innerhook := newAuthenticatorFromInnerHook(config.InnerHookFunc)
		authenticators = append(authenticators, innerhook)
		hasTokenAuth = true
	}
...
// newAuthenticatorFromInnerHook returns an authenticator.Request
func newAuthenticatorFromInnerHook(hook innerhook.InnerHookFunc) authenticator.Request {
	return bearertoken.New(innerhook.InnerHookAuthenticator{HookFunc: hook})
}
