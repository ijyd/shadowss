...
type Registry interface {
	...
	UpdateUserToken(ctx freezerapi.Context, token *api.UserToken) error
}
...
func (s *storage) UpdateUserToken(ctx freezerapi.Context, token *api.UserToken) error {
	_, _, err := s.Update(ctx, token.Name, rest.DefaultUpdatedObjectInfo(token, api.Scheme))
	return err
}
